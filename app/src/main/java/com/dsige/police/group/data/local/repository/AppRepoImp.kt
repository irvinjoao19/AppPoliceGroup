...
import com.dsige.police.group.data.local.AppDataBase
import com.dsige.police.group.helper.MensajeDetalle
...
class AppRepoImp(private val apiService: ApiService, private val dataBase: AppDataBase) :
    ...
    override fun saveSync(s: Sync): Completable {
        return Completable.fromAction {
            ...
            val e: List<Empresa>? = s.empresas
            if (e != null) {
                dataBase.empresaDao().insertEmpresaListTask(e)
            }
        }
    }
    ...
    override fun getOts(
        empresaId: Int, estadoId: Int, s: String
    ): LiveData<PagedList<ParteDiario>> {
        return if (empresaId == 0) {
            if (s.isEmpty()) {
                dataBase.parteDiarioDao().getOts(estadoId).toLiveData(
                    Config(pageSize = 20, enablePlaceholders = true)
                )
            } else {
                dataBase.parteDiarioDao().getOts(estadoId, s).toLiveData(
                    Config(pageSize = 20, enablePlaceholders = true)
                )
            }
        } else {
            if (s.isEmpty()) {
                dataBase.parteDiarioDao().getOts(estadoId, empresaId).toLiveData(
                    Config(pageSize = 20, enablePlaceholders = true)
                )
            } else {
                dataBase.parteDiarioDao().getOts(estadoId, empresaId, s).toLiveData(
                    Config(pageSize = 20, enablePlaceholders = true)
                )
            }
        }
    ...
    override fun getEmpresas(): LiveData<List<Empresa>> {
        return dataBase.empresaDao().getEmpresas()
    }
}