...
interface ParteDiarioDao {
    ...
    @Query("SELECT * FROM ParteDiario WHERE estadoId=:e AND empresaId =:m")
    fun getOts(e: Int,m:Int): DataSource.Factory<Int, ParteDiario>
    ...
    @Query("SELECT * FROM ParteDiario WHERE estadoId=:e AND empresaId =:m AND nroObraTD LIKE :s")
    fun getOts(e: Int,m:Int, s: String): DataSource.Factory<Int, ParteDiario>
    ...
    @Query("UPDATE ParteDiario SET identity =:codigoRetorno , estado = 0 , estadoId =6 , nombreEstado=:e WHERE parteDiarioId=:codigoBase")
    fun updateEnabledOt(codigoBase: Int, codigoRetorno: Int,e:String)
